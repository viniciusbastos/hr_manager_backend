...
router.get('/user/search/:mat', async (req, res, next) => {
  try {
    ...
    const user: any =
      await prisma.$queryRaw`SELECT "User".id, "User"."name", "User".posto, "User".mat, "Vacation"."month", "Vacation"."year", "Vacation"."startAt", "Vacation"."finishAt", "Vacation".approved, "Vacation"."period" FROM "User" INNER JOIN "Vacation" on "User".id = "Vacation"."belongsToId" WHERE "mat" LIKE ${mat} `
